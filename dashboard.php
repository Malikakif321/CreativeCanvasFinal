...
// Fetch user bookings
$bookings = [];
$sql = "SELECT exhibition, date FROM bookings WHERE user_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $user_id);
$stmt->execute();
      ...
      <?php if (count($bookings) > 0): ?>
        <ul>
          <?php foreach ($bookings as $booking): ?>
            <li><strong><?php echo htmlspecialchars($booking['exhibition']); ?></strong> on <?php echo htmlspecialchars($booking['date']); ?></li>
          <?php endforeach; ?>
        </ul>
      <?php else: ?>
